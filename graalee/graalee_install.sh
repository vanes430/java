...
for VERSION in "${VERSIONS[@]}"; do
    ...
    sed -e "s|ARG JAVA_VERSION=21|ARG JAVA_VERSION=${VERSION}|" \
        ...
        graalee_debian.Dockerfile > "../target/graalee_debian_${VERSION}.Dockerfile"
    echo "Generated target/graalee_debian_${VERSION}.Dockerfile"

    # Rocky
    sed -e "s|ARG JAVA_VERSION=21|ARG JAVA_VERSION=${VERSION}|" \
        -e "s|ARG DOWNLOAD_URL_X64|ARG DOWNLOAD_URL_X64=${URL_X64}|" \
        -e "s|ARG DOWNLOAD_URL_ARM64|ARG DOWNLOAD_URL_ARM64=${URL_ARM64}|" \
        graalee_rocky.Dockerfile > "../target/graalee_rocky_${VERSION}.Dockerfile"
    echo "Generated target/graalee_rocky_${VERSION}.Dockerfile"
done
